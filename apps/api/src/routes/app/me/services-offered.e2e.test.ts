...
const makeService = (
  input: ServiceOfferedCreateInput,
  overrides: Partial<ServiceOffered> = {}
): ServiceOffered => ({
...
const makeInMemoryServiceRepo = (services: Array<ServiceOffered>) => ({
  listByUserId: (userId: string) =>
    Effect.succeed(
      services.filter((service) => service.userId === userId && service.deletedAt === null)
    ),
  findByIdForUser: (id: string, userId: string) => {
    const service = services.find(
      (service) => service.id === id && service.userId === userId && service.deletedAt === null
    );
    return service
...
describe('/me/services-offered', () => {
  ...
  it('soft deletes services using deletedAt', async () => {
    const service = makeService(
      { userId: 'provider-1', name: 'Weekend childcare', hourlyRateCents: 3000, currency: 'CAD' },
      { id: 'service-1' }
    );
    const services = [service];
    const app = makeApp({ services });

    const deleteRes = await app.request('/api/v1/me/services-offered/service-1', {
      method: 'DELETE'
    });
