...
export class ServiceNeededRepo extends Context.Tag("@repo/db/ServiceNeededRepo")<
  ServiceNeededRepo,
  {
    ...
    findByIdForUser: (id: string, userId: string) => Effect.Effect<ServiceNeeded, SqlError | DBNotFoundError>;
    create: (input: ServiceNeededCreateInput) => Effect.Effect<ServiceNeeded, SqlError>;
    updateByIdForUser: (id: string, userId: string, input: ServiceNeededUpdateInput) => Effect.Effect<ServiceNeeded, SqlError | DBNotFoundError>;
    softDeleteByIdForUser: (id: string, userId: string) => Effect.Effect<ServiceNeeded, SqlError | DBNotFoundError>;
  }
>() { }
    ...
    return {
      ...
      findByIdForUser: (id, userId) =>
        db
          .select()
          .from(serviceNeeded)
          .where(and(eq(serviceNeeded.id, id), eq(serviceNeeded.userId, userId), isNull(serviceNeeded.deletedAt)))
          .limit(1)
...
export const EmptyServiceNeededRepoTest = makeServiceNeededRepoTest({
  ...
  softDeleteByIdForUser: () => Effect.fail(new DBNotFoundError({ entity: "serviceNeeded", value: "" })),
});
