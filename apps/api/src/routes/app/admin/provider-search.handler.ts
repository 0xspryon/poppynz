...
type AdminProviderSearchRouteError = Effect.Effect.Error<
  ReturnType<typeof scheduleProviderSearchReindexRouteProgram>
>;
  ...
  switch (error._tag) {
    ...
    case 'ProviderSearchQueueError':
      return c.json(
        {
          error: {
            code: 'PROVIDER_SEARCH_REINDEX_SCHEDULE_FAILED' as const,
            message: 'Unable to schedule provider search reindex.'
          }
        },
        502
      );
...
export async function scheduleProviderSearchReindexHandler(c: HonoContext<HonoEnv>) {
  ...
  const exit = await runtime.runPromiseExit(
    scheduleProviderSearchReindexRouteProgram(c.req.raw.headers)
  );
