...
const user = (overrides: Partial<User> = {}): User => ({
  id: "family-1", name: "Family", email: "family@example.com", emailVerified: true, image: null,
...
const session = (): Session => ({
  ...
  createdAt: new Date("2026-06-12T00:00:00.000Z"), updatedAt: new Date("2026-06-12T00:00:00.000Z"),
  ipAddress: null, userAgent: null, userId: "family-1", impersonatedBy: null, activeOrganizationId: null,
});

const service = (overrides: Partial<ServiceNeeded> = {}): ServiceNeeded => ({
  id: "need-1", userId: "family-1", catalogueServiceId: null, name: "Childcare", description: null,
...
const makeLayer = (options: {
  ...
  onUpdate?: (input: { name?: string; description?: string | null; catalogueServiceId?: string | null }) => void;
} = {}) => {
...
describe("services needed route programs", () => {
  it("lists needed services for the authenticated family with the family limit", async () => {
    const result = await Effect.runPromise(listServicesNeededRouteProgram(new Headers()).pipe(Effect.provide(makeLayer())));
    expect(result.services).toEqual([expect.objectContaining({ id: "need-1", name: "Childcare", createdAt: "2026-06-12T00:00:00.000Z" })]);
  ...
  it("reads the family limit from SERVICES_NEEDED_MAX_PER_FAMILY", async () => {
    const result = await Effect.runPromise(
      listServicesNeededRouteProgram(new Headers()).pipe(
        ...
        Effect.withConfigProvider(ConfigProvider.fromMap(new Map([["SERVICES_NEEDED_MAX_PER_FAMILY", "5"]]))),
      ),
    );
  ...
  it("rejects creating a need once the family limit is reached", async () => {
    ...
    const exit = await Effect.runPromise(
      createServiceNeededRouteProgram(contextWithJson({ name: "One too many" }), new Headers()).pipe(
  ...
  it("drops rate fields the schema does not know", async () => {
    ...
    await Effect.runPromise(
      createServiceNeededRouteProgram(contextWithJson({ name: "Tutoring", hourlyRateCents: 3000 }), new Headers()).pipe(
  ...
  it("rejects users without service-needed permission", async () => {
    const exit = await Effect.runPromise(listServicesNeededRouteProgram(new Headers()).pipe(Effect.provide(makeLayer({ hasPermission: false })), Effect.exit));
  ...
  it("soft deletes a needed service and schedules a family search reconcile", async () => {
    const reconciledUserIds: Array<string> = [];
    const result = await Effect.runPromise(
      deleteServiceNeededRouteProgram(new Headers(), "need-1").pipe(Effect.provide(makeLayer({ reconciledUserIds }))),
    );
...
describe("services needed catalogue rules", () => {
  ...
  const hiddenItem = catalogueItem({ id: "0197b3a0-0000-7000-8000-000000000002", name: "Homework tutoring", category: "Tutoring", isLive: false });
  ...
  it("rejects creating a need linked to an unknown catalogue item", async () => {
    const exit = await Effect.runPromise(
      createServiceNeededRouteProgram(
        contextWithJson({ name: "Babysitting", catalogueServiceId: "0197b3a0-0000-7000-8000-00000000dead" }),
        new Headers(),
  ...
  it("keeps an existing link to a now-hidden item when editing other fields", async () => {
    ...
    const result = await Effect.runPromise(
      updateServiceNeededRouteProgram(contextWithJson({ description: "Weekday afternoons" }), new Headers(), existing.id).pipe(
        Effect.provide(makeLayer({ existingService: existing, catalogueItems: [hiddenItem] })),
      ),
    );
    expect(result).toMatchObject({ description: "Weekday afternoons", catalogueServiceId: hiddenItem.id });
  });

  it("rejects re-linking a need to a hidden catalogue item", async () => {
    const existing = service({ catalogueServiceId: null });
    const exit = await Effect.runPromise(
      updateServiceNeededRouteProgram(contextWithJson({ catalogueServiceId: hiddenItem.id }), new Headers(), existing.id).pipe(
        Effect.provide(makeLayer({ existingService: existing, catalogueItems: [hiddenItem] })),
        Effect.exit,
      ),
    );
  ...
  it("unlinks a need from the catalogue with an explicit null", async () => {
    ...
    const result = await Effect.runPromise(
      updateServiceNeededRouteProgram(contextWithJson({ catalogueServiceId: null }), new Headers(), existing.id).pipe(
  ...
  it("clears the description with an explicit null", async () => {
    ...
    await Effect.runPromise(
      updateServiceNeededRouteProgram(contextWithJson({ description: null }), new Headers(), existing.id).pipe(
