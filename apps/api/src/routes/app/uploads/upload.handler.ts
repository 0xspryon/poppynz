...
const allowedPublicImageContentTypes = new Set(['image/jpeg', 'image/png', 'image/webp']);
...
const validateUploadInput = (userRole: Roles | null, input: UploadPresignInput) =>
  Effect.gen(function* () {
    ...
    if (input.target === 'kyc-document') {
      ...
      const documentType = yield* documentTypeRepo
        .findActiveById(input.documentTypeId)
        .pipe(
          Effect.mapError(
            () => new UploadValidationError({ message: 'KYC document type was not found.' })
          )
        );
      ...
      if (documentType.appliesToRole !== userRole) {
        return yield* Effect.fail(
          new UploadValidationError({
            message: 'KYC document type is not available for this role.'
          })
        );
      }
      ...
      if (!allowedKycContentTypes.has(input.contentType)) {
        return yield* Effect.fail(
          new UploadValidationError({ message: 'Unsupported KYC document file type.' })
        );
      }

      if (input.sizeBytes > maxKycDocumentSizeBytes) {
        return yield* Effect.fail(
          new UploadValidationError({ message: 'KYC document is too large.' })
        );
      }
    ...
    if (!allowedPublicImageContentTypes.has(input.contentType)) {
      return yield* Effect.fail(
        new UploadValidationError({ message: 'Unsupported public image file type.' })
      );
    }

    if (input.sizeBytes > maxPublicImageSizeBytes) {
      return yield* Effect.fail(
        new UploadValidationError({ message: 'Public image is too large.' })
      );
    }
  });
...
export const createUploadPresignProgram = (
...
export const createUploadPresignRouteProgram = (c: HonoContext<HonoEnv>, headers: Headers) =>
  Effect.gen(function* () {
    ...
    const userAndSession = yield* requirePermissions(headers, { profile: ['update'] })(
      authenticated
    );
...
export type UploadRouteError = Effect.Effect.Error<
  ReturnType<typeof createUploadPresignRouteProgram>
>;
...
const exitToResponse = <TUpload>(
  c: HonoContext<HonoEnv>,
  exit: Exit.Exit<TUpload, UploadRouteError>
) =>
  Exit.match(exit, {
...
export async function createUploadPresignHandler(c: HonoContext<HonoEnv>) {
  ...
  const exit = await runtime.runPromiseExit(createUploadPresignRouteProgram(c, headers));
