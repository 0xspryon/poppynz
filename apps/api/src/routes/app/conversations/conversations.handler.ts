...
export class ConversationPairInvalidError extends Data.TaggedError("ConversationPairInvalidError")<{}> { }
...
export class InvalidReachoutServicesError extends Data.TaggedError("InvalidReachoutServicesError")<{}> { }

export class ConversationNotFoundError extends Data.TaggedError("ConversationNotFoundError")<{}> { }

export class ConversationNotPendingError extends Data.TaggedError("ConversationNotPendingError")<{}> { }

export class NotConversationReceiverError extends Data.TaggedError("NotConversationReceiverError")<{}> { }
...
function mapConversationRepoError<A, R>(
  ...
  return effect.pipe(Effect.catchTag("SqlError", (cause) => Effect.fail(new ConversationRepoError({ cause }))));
}
...
const reachoutIgnoredCutoff = reachoutConfig.pipe(
  ...
  Effect.map(({ ignoreCooldownDays }) => new Date(Date.now() - ignoreCooldownDays * 24 * 60 * 60 * 1000)),
);
...
export const toConversationResponse = (row: ConversationWithContext, viewerUserId: string) => {
  ...
  return {
    ...
    reachout: {
      ...
      services: row.reachoutServices.map((service) => ({ serviceId: service.serviceId, name: service.name })),
  ...
  Effect.gen(function* () {
    ...
    if (existing) {
      if (isExpiredIgnore(existing, ignoredCutoff)) {
        ...
        yield* conversationRepo.softDeleteById(existing.id).pipe((errors) => mapConversationRepoError(errors));
...
export const unreadCountProgram = (userAndSession: UserAndSession) =>
  listConversationsProgram(userAndSession).pipe(Effect.map((result) => ({ total: result.unreadTotal })));

export const lookupConversationProgram = (userAndSession: UserAndSession, counterpartUserId: string) =>
  Effect.gen(function* () {
    const conversationRepo = yield* ConversationRepo;
...
const loadParticipantConversation = (conversationId: string, viewerUserId: string) =>
  Effect.gen(function* () {
    ...
    if (
      !conversation ||
      conversation.familyUserId !== viewerUserId && conversation.providerUserId !== viewerUserId ||
...
export const sendMessageProgram = (
  ...
  Effect.gen(function* () {
    ...
    const recipientUserId =
      conversation.familyUserId === viewer.id ? conversation.providerUserId : conversation.familyUserId;
...
export const markConversationReadProgram = (userAndSession: UserAndSession, conversationId: string) =>
  Effect.gen(function* () {
    const conversationRepo = yield* ConversationRepo;
    const viewer = userAndSession.user;
    const conversation = yield* loadParticipantConversation(conversationId, viewer.id);
...
export const createReachoutRouteProgram = (c: HonoContext<HonoEnv>, headers: Headers) =>
  Effect.gen(function* () {
    ...
    const userAndSession = yield* requirePermissions(headers, { conversation: ["write"] })(authenticated);
...
export const listConversationsRouteProgram = (headers: Headers) =>
  Effect.gen(function* () {
    const authenticated = yield* authenticate(headers);
    const userAndSession = yield* requirePermissions(headers, { conversation: ["read"] })(authenticated);
...
const conversationRouteErrorToResponse = (c: HonoContext<HonoEnv>, error: ConversationRouteError) => {
  switch (error._tag) {
    ...
    case "ConversationRepoError":
      return c.json(
        { error: { code: "CONVERSATION_LOOKUP_FAILED" as const, message: "Unable to process the conversation." } },
        500,
      );
    case "RecipientNotFoundError":
      return c.json(
        { error: { code: "RECIPIENT_NOT_FOUND" as const, message: "This person could not be found." } },
        404,
      );
...
const exitToResponse = <TData>(c: HonoContext<HonoEnv>, exit: Exit.Exit<TData, ConversationRouteError>) =>
  Exit.match(exit, {
