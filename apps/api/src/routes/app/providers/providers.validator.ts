...
export class ProviderSearchRequestValidationError extends Data.TaggedError(
  'ProviderSearchRequestValidationError'
)<{ message: string }> {}

const sortValues = new Set<ProviderSearchSort>([
  'relevance',
  'distance',
  'price_asc',
  'price_desc',
...
const optionalNumber = (value: string | undefined, name: string) => {
  ...
  return Number.isFinite(parsed)
    ...
    : Effect.fail(
        new ProviderSearchRequestValidationError({ message: `${name} must be a number.` })
      );
};
...
const boundedPositiveInteger = (
  ...
  if (!/^\d+$/.test(value.trim())) {
    return Effect.fail(
      new ProviderSearchRequestValidationError({ message: `${name} must be a positive integer.` })
    );
  }
  ...
  if (parsed > max) {
    return Effect.fail(
      new ProviderSearchRequestValidationError({ message: `${name} must be ${max} or less.` })
    );
  }
...
export const validateProviderSearchQuery = (query: Record<string, string | undefined>) =>
  Effect.gen(function* () {
    ...
    const [page, perPage, radiusKm, minHourlyRateCents, maxHourlyRateCents, sort] =
      yield* Effect.all([
    ...
    if (sort === 'distance' && radiusKm === undefined) {
      return yield* Effect.fail(
        new ProviderSearchRequestValidationError({ message: 'sort=distance requires radiusKm.' })
      );
    }
