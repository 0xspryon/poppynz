...
import { getOnboardingHistoryProgram, getOnboardingProgram, OnboardingRoleError } from "./onboarding.handler";
...
const makeLayer = (options: {
  ...
  return Layer.mergeAll(
    ...
    makeServiceOfferedRepoTest({
      ...
      findByIdForUser: (id) => Effect.fail(new DBNotFoundError({ entity: "serviceOffered", value: id })),
    ...
    makeApprovalRepoTest({
      ...
      findCurrentByUserId: (userId) => {
        const current = approvals.find((entry) => entry.userId === userId && entry.expiresAt > new Date());
        return current
...
describe("getOnboardingProgram", () => {
  it("computes per-step state and progress for a provider mid-onboarding", async () => {
    const types = [
      ...
      documentType({ id: "document-type-3", name: "Driving record", isOptional: true, isFetchable: true }),
      documentType({ id: "document-type-4", name: "Admin-only doc", appliesToRole: "admin" }),
    ];
    const result = await Effect.runPromise(
      getOnboardingProgram(userAndSession()).pipe(
        Effect.provide(makeLayer({ documentTypes: types, documents: [kycDocument()], services: [] })),
      ),
    );

    expect(result.steps.profile).toEqual({ complete: true, missingFields: [] });
    expect(result.steps.documents).toEqual({ complete: false, requiredSubmitted: 1, requiredTotal: 2 });
    ...
    expect(result.documents[0]).toMatchObject({ status: "submitted", document: { id: "document-1" } });
  ...
  it("returns the current approval when one is active", async () => {
    const result = await Effect.runPromise(
      getOnboardingProgram(userAndSession()).pipe(
        Effect.provide(makeLayer({
          approvals: [approval()],
          requests: [approvalRequest({ status: "approved", reviewedAt: new Date("2026-07-06T00:00:00.000Z") })],
  ...
  it("returns requests and approvals for the provider", async () => {
    const result = await Effect.runPromise(
      getOnboardingHistoryProgram(userAndSession()).pipe(
        Effect.provide(makeLayer({
          requests: [
            approvalRequest({ id: "request-2", status: "rejected", reason: "Police clearance is too old.", reviewedAt: new Date("2026-07-05T00:00:00.000Z") }),
            approvalRequest(),
          ],
          approvals: [approval()],
        })),
    ...
    expect(result.requests[0]).toMatchObject({ id: "request-2", status: "rejected", reason: "Police clearance is too old." });
