...
const makeLayer = (options: {
  ...
  return Layer.mergeAll(
    ...
    makeKycDocumentTypeRepoTest({
      ...
      findActiveById: (id) => currentType?.id === id && currentType.deletedAt === null
        ? Effect.succeed(currentType)
        : Effect.fail(new DBNotFoundError({ entity: "kycDocumentType", value: id })),
      create: (input) => {
        options.onCreateType?.(input);
        return options.createTypeError ? Effect.fail(options.createTypeError) : Effect.succeed(documentType(input));
      },
      update: (id, input) => {
        options.onUpdateType?.(input);
        return currentType?.id === id ? Effect.succeed({ ...currentType, ...input }) : Effect.fail(new DBNotFoundError({ entity: "kycDocumentType", value: id }));
      },
      softDelete: (id) => currentType?.id === id
        ? Effect.succeed({ ...currentType, deletedAt: new Date("2026-06-13T00:00:00.000Z") })
        : Effect.fail(new DBNotFoundError({ entity: "kycDocumentType", value: id })),
    }),
    makeKycDocumentRepoTest({
      findByIdWithType: (id) => currentDocument?.id === id && currentType
      ...
      submit: (input) => {
        ...
        return options.submitError ? Effect.fail(options.submitError) : Effect.succeed(kycDocument(input));
      },
      updateExpiryDate: (id, expiryDate) => currentDocument?.id === id
...
describe("KYC route programs", () => {
  it("creates a document type with default service-provider role", async () => {
    ...
    const result = await Effect.runPromise(
      createKycDocumentTypeRouteProgram(contextWithJson({ name: " Vulnerable Sector Check ", isOptional: false, requiresExpiryDate: true }), new Headers()).pipe(
        Effect.provide(makeLayer({ onCreateType: (input) => created.push(input) })),
      ),
    );
  ...
  it("rejects document type writes without permission", async () => {
    const exit = await Effect.runPromise(
      createKycDocumentTypeRouteProgram(contextWithJson({ name: "ID", isOptional: false, requiresExpiryDate: false }), new Headers()).pipe(
  ...
  it("updates and soft-deletes document types", async () => {
    const updates: Array<KycDocumentTypeUpdateInput> = [];
    const updated = await Effect.runPromise(
      updateKycDocumentTypeRouteProgram(contextWithJson({ isOptional: true }), new Headers(), "document-type-1").pipe(
        Effect.provide(makeLayer({ onUpdateType: (input) => updates.push(input) })),
      ),
    );
    const deleted = await Effect.runPromise(deleteKycDocumentTypeRouteProgram(new Headers(), "document-type-1").pipe(Effect.provide(makeLayer())));
  ...
  it("submits a required KYC document with expiry", async () => {
    ...
    const result = await Effect.runPromise(
      submitKycDocumentRouteProgram(contextWithJson({
        documentTypeId: "document-type-1",
        filename: "id.pdf",
        fileKey: "users/provider-1/kyc/document-type-1/id.pdf",
        expiryDate: "2027-06-12T00:00:00.000Z",
      }), new Headers()).pipe(Effect.provide(makeLayer({ onSubmit: (input) => submitted.push(input) }))),
    );

    expect(result).toMatchObject({ documentTypeId: "document-type-1", filename: "id.pdf", status: "submitted" });
    expect(submitted[0]).toMatchObject({ userId: "provider-1", documentTypeId: "document-type-1", fileKey: "users/provider-1/kyc/document-type-1/id.pdf" });
  });

  it("rejects missing required expiry and foreign file keys", async () => {
    const missingExpiry = await Effect.runPromise(
      submitKycDocumentRouteProgram(contextWithJson({ documentTypeId: "document-type-1", filename: "id.pdf", fileKey: "users/provider-1/kyc/document-type-1/id.pdf" }), new Headers()).pipe(
        Effect.provide(makeLayer()),
        Effect.exit,
      ),
    );
    const foreignKey = await Effect.runPromise(
      submitKycDocumentRouteProgram(contextWithJson({ documentTypeId: "document-type-1", filename: "id.pdf", fileKey: "users/other/kyc/document-type-1/id.pdf", expiryDate: "2027-06-12T00:00:00.000Z" }), new Headers()).pipe(
        Effect.provide(makeLayer()),
        Effect.exit,
      ),
    );
  ...
  it("rejects family users submitting service-provider KYC docs", async () => {
    const exit = await Effect.runPromise(
      submitKycDocumentRouteProgram(contextWithJson({ documentTypeId: "document-type-1", filename: "id.pdf", fileKey: "users/provider-1/kyc/document-type-1/id.pdf", expiryDate: "2027-06-12T00:00:00.000Z" }), new Headers()).pipe(
  ...
  it("updates admin KYC document expiry and rejects null when required", async () => {
    const updated = await Effect.runPromise(
      updateAdminKycDocumentRouteProgram(contextWithJson({ expiryDate: "2027-07-01T00:00:00.000Z" }), new Headers(), "kyc-document-1").pipe(Effect.provide(makeLayer())),
    );
    const nullExpiry = await Effect.runPromise(
      updateAdminKycDocumentRouteProgram(contextWithJson({ expiryDate: null }), new Headers(), "kyc-document-1").pipe(Effect.provide(makeLayer()), Effect.exit),
    );
  ...
  it("translates missing KYC records and repo failures", async () => {
    const missingType = await Effect.runPromise(
      submitKycDocumentRouteProgram(contextWithJson({ documentTypeId: "missing", filename: "id.pdf", fileKey: "users/provider-1/kyc/missing/id.pdf", expiryDate: "2027-06-12T00:00:00.000Z" }), new Headers()).pipe(
        Effect.provide(makeLayer({ type: null })),
        Effect.exit,
      ),
    );
