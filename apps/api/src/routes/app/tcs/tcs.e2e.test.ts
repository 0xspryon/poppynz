  ...
  return {
    ...
    findActiveById: (id: string) => {
      ...
      return document
        ...
        : Effect.fail(new DBNotFoundError({ entity: 'tcDocument', value: id }));
    },
    findActiveBySlug: (slug: string) => {
      const document = activeDocuments().find((candidate) => candidate.slug === slug);
      return document
        ? Effect.succeed(document)
        : Effect.fail(new DBNotFoundError({ entity: 'tcDocument', value: slug }));
    },
    ...
    createDraft: (documentId: string, input: TcDraftInput) => {
      ...
      if (!document)
        return Effect.fail(new DBNotFoundError({ entity: 'tcDocument', value: documentId }));
      const latest = versions
        .filter((version) => version.documentId === documentId)
        .sort((a, b) => b.version - a.version)[0];
    ...
    updateDraft: (documentId: string, input: TcDraftUpdateInput) => {
      const draft = versions.find(
        (version) => version.documentId === documentId && version.publishedAt === null
      );
      if (!draft)
        return Effect.fail(new DBNotFoundError({ entity: 'tcDocumentVersion', value: documentId }));
      ...
      Effect.succeed(
        publishedForRole(role).filter(
          (entry) =>
            !acceptances.some(
              (acceptance) =>
                acceptance.userId === userId && acceptance.versionId === entry.version.id
            )
        )
      ),
    ...
    insertAcceptances: (userId: string, items: Array<TcAcceptanceInput>) => {
      const inserted = items
        .filter(
          (item) =>
            !acceptances.some(
              (existing) => existing.userId === userId && existing.versionId === item.versionId
            )
        )
        .map((item) => ({
          ...item,
...
describe('/tcs', () => {
  it('serves published documents for a requested role without a session', async () => {
    ...
    const app = makeApp({
      ...
      documents: [doc, spDoc],
      versions: [
        makeVersion({ id: 'v-1', documentId: 'doc-1' }),
        makeVersion({ id: 'v-sp', documentId: 'doc-sp' })
      ]
    });
  ...
  it('runs the full accept → publish v2 → re-accept loop', async () => {
    ...
    expect(pendingBody1[0]).toMatchObject({
      slug: 'terms_of_service',
      versionId: 'v-1',
      version: 1
    });
    ...
    expect((await accept1.json()).accepted[0]).toMatchObject({
      slug: 'terms_of_service',
      version: 1,
      userId: 'user-1'
    });
    ...
    const draftRes = await adminApp.request('/api/v1/admin/tcs/doc-1/draft', {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        description: 'Clarified fees',
        content: '# Terms v2',
        checkboxLabel: 'I agree to the new terms.'
      })
    });
    ...
    const publishRes = await adminApp.request('/api/v1/admin/tcs/doc-1/publish', {
      method: 'POST'
    });
    ...
    expect(pendingBody3[0]).toMatchObject({
      version: 2,
      checkboxLabel: 'I agree to the new terms.'
    });
    ...
    const accept2 = await familyApp.request('/api/v1/me/tcs/accept', {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        acceptances: [{ slug: 'terms_of_service', versionId: pendingBody3[0].versionId }]
      })
    });
  ...
  it('lets an admin manage documents end to end', async () => {
    ...
    const draftRes = await app.request(`/api/v1/admin/tcs/${created.id}/draft`, {
      ...
      body: JSON.stringify({
        description: 'Initial version',
        content: '# Privacy',
        checkboxLabel: 'I acknowledge the policy.'
      })
    });
    expect(draftRes.status).toBe(200);
    ...
    const publishRes = await app.request(`/api/v1/admin/tcs/${created.id}/publish`, {
      method: 'POST'
    });
    expect(publishRes.status).toBe(200);
