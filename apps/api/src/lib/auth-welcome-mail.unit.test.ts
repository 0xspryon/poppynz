...
describe("sendWelcomeMailEffect", () => {
  ...
  it("sends the provider next-steps mail to new service-provider accounts", async () => {
    ...
    await Effect.runPromise(
      sendWelcomeMailEffect({ email: "provider@example.com", name: null, role: "service-provider" }).pipe(
        Effect.provide(layer),
      ),
    );
  ...
  it("sends nothing for admin or unknown roles", async () => {
    ...
    await Effect.runPromise(
      Effect.all([
        sendWelcomeMailEffect({ email: "admin@example.com", role: "admin" }).pipe(Effect.provide(layer)),
        sendWelcomeMailEffect({ email: "nobody@example.com", role: null }).pipe(Effect.provide(layer)),
  ...
  it("does not fail account creation when the mail send fails", async () => {
    ...
    await expect(
      Effect.runPromise(
        sendWelcomeMailEffect({ email: "family@example.com", role: "family" }).pipe(Effect.provide(layer)),
      ),
