...
export class GooglePlaces extends Context.Tag('@repo/google/GooglePlaces')<
  GooglePlaces,
  {
    lookupPlaceById: (
      ...
      GooglePlaceLocation,
      GooglePlacesError | GooglePlaceNotFoundError | GooglePlaceInvalidError
    >;
    autocompletePlaces: (
      query: string
    ) => Effect.Effect<Array<GooglePlaceSuggestion>, GooglePlacesError>;
  }
>() {}
...
const normalizePlaceDetails = (placeId: string, result: PlaceDetailsResponse) => {
  ...
  if (typeof latitude !== 'number' || typeof longitude !== 'number') {
    return Effect.fail(
      new GooglePlaceInvalidError({ placeId, message: 'Google place is missing coordinates.' })
    );
  }
