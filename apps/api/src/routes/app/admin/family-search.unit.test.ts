...
import { scheduleFamilySearchReindexRouteProgram } from "./family-search.handler";
...
const makeLayer = (options: { hasPermission?: boolean; onReindex?: () => void } = {}) => {
  ...
  return Layer.mergeAll(
    ...
    makeFamilySearchQueueTest({
      enqueueReconcile: () => Effect.succeed({ id: "job-1", name: "reconcile-family" }),
      enqueueReindex: () => {
        options.onReindex?.();
        return Effect.succeed({ id: "job-3", name: "reindex-all-families" });
      },
    }),
  );
};

describe("admin family search route program", () => {
  it("schedules a family search reindex job", async () => {
    const onReindex = vi.fn();
    const result = await Effect.runPromise(scheduleFamilySearchReindexRouteProgram(new Headers()).pipe(Effect.provide(makeLayer({ onReindex }))));
  ...
  it("requires reindex permission", async () => {
    const exit = await Effect.runPromise(scheduleFamilySearchReindexRouteProgram(new Headers()).pipe(Effect.provide(makeLayer({ hasPermission: false })), Effect.exit));
