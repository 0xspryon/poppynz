...
const makeInMemoryCatalogueRepo = (items: Array<ServiceCatalogueItem>) => ({
  ...
  create: (input: ServiceCatalogueItemCreateInput) => {
    const item = makeItem({ ...input, id: "item-new", currency: input.currency ?? "CAD", isLive: input.isLive ?? true });
    items.push(item);
    return Effect.succeed(item);
  },
  update: (id: string, input: ServiceCatalogueItemUpdateInput) => {
    const item = items.find((item) => item.id === id && item.deletedAt === null);
    if (!item) return Effect.fail(new DBNotFoundError({ entity: "serviceCatalogueItem", value: id }));
...
const makeApp = (options: { authSession?: AuthSession | null; hasPermission?: boolean; user?: User; items?: Array<ServiceCatalogueItem> } = {}) => {
...
describe("/service-catalogue", () => {
  it("lets an admin create, update, and soft delete catalogue items", async () => {
    ...
    const createRes = await app.request("/api/v1/admin/service-catalogue", {
      ...
      body: JSON.stringify({ name: "Weekend babysitting", category: "Childcare", baseHourlyRateCents: 1900 }),
    });

    expect(createRes.status).toBe(200);
    const created = await createRes.json();
    expect(created).toMatchObject({ name: "Weekend babysitting", baseHourlyRateCents: 1900, currency: "CAD", isLive: true });
    ...
    const deleteRes = await app.request(`/api/v1/admin/service-catalogue/${created.id}`, { method: "DELETE" });
  ...
  it("shows providers live items only", async () => {
    const items = [makeItem({ id: "item-live" }), makeItem({ id: "item-hidden", name: "Homework tutoring", category: "Tutoring", isLive: false })];
  ...
  it("rejects mutations without the serviceCatalogue write permission", async () => {
    const app = makeApp({ user: makeUser({ id: "provider-1", role: "service-provider" }), hasPermission: false });
