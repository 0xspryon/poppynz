...
const makeInMemoryApprovalRequestRepo = (requests: Array<ApprovalRequest>) => ({
  ...
  listByUserId: (userId: string) =>
    Effect.succeed(requests.filter((request) => request.userId === userId)),
  findById: (id: string) => {
  ...
  findSubmittedByUserId: (userId: string) => {
    const request = requests.find(
      (request) => request.userId === userId && request.status === 'submitted'
    );
    return request
      ? Effect.succeed(request)
...
const makeInMemoryApprovalRepo = (approvals: Array<Approval>) => ({
  ...
  listByUserId: (userId: string) =>
    Effect.succeed(approvals.filter((approval) => approval.userId === userId)),
  revoke: (id: string, reason: string) => {
    const target = approvals.find(
      (approval) => approval.id === id && approval.status === 'approved'
    );
