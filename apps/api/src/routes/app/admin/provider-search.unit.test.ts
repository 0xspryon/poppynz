...
describe("admin provider search route program", () => {
  it("schedules a provider search reindex job", async () => {
    ...
    const result = await Effect.runPromise(scheduleProviderSearchReindexRouteProgram(new Headers()).pipe(Effect.provide(makeLayer({ onReindex }))));
  ...
  it("requires reindex permission", async () => {
    const exit = await Effect.runPromise(scheduleProviderSearchReindexRouteProgram(new Headers()).pipe(Effect.provide(makeLayer({ hasPermission: false })), Effect.exit));
