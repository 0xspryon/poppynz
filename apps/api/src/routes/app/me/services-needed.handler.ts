...
class ServicesNeededLimitReachedError extends Data.TaggedError('ServicesNeededLimitReachedError')<{
  max: number;
}> {}
...
export const listServicesNeededRouteProgram = (headers: Headers) =>
  Effect.gen(function* () {
    ...
    const userAndSession = yield* requirePermissions(headers, { serviceNeeded: ['read'] })(
      authenticated
    );
...
export const createServiceNeededRouteProgram = (c: HonoContext<HonoEnv>, headers: Headers) =>
  Effect.gen(function* () {
    ...
    const authenticated = yield* authenticate(headers);
    const userAndSession = yield* requirePermissions(headers, { serviceNeeded: ['write'] })(
      authenticated
    );
...
export const updateServiceNeededRouteProgram = (
  ...
  switch (error._tag) {
    case 'DBNotFoundError':
      return c.json(
        {
          error: {
            code: 'SERVICE_NEEDED_NOT_FOUND' as const,
            message: 'Service needed was not found.'
          }
        },
        404
      );
    case 'SqlError':
      return c.json(
        {
          error: {
            code: 'SERVICE_NEEDED_REPO_ERROR' as const,
            message: 'Unable to process service needed request.'
          }
        },
        500
      );
...
export async function listServicesNeededHandler(c: HonoContext<HonoEnv>) {
  ...
  const exit = await runtime.runPromiseExit(listServicesNeededRouteProgram(headers));
...
export async function createServiceNeededHandler(c: HonoContext<HonoEnv>) {
  const runtime = c.get('runtime');
  const headers = c.req.raw.headers;
  const exit = await runtime.runPromiseExit(createServiceNeededRouteProgram(c, headers));
...
export async function updateServiceNeededHandler(c: HonoContext<HonoEnv>) {
  ...
  const exit = await runtime.runPromiseExit(updateServiceNeededRouteProgram(c, headers, serviceId));
...
export async function deleteServiceNeededHandler(c: HonoContext<HonoEnv>) {
  ...
  const headers = c.req.raw.headers;
  const serviceId = c.req.param('id') ?? '';
  const exit = await runtime.runPromiseExit(deleteServiceNeededRouteProgram(headers, serviceId));
