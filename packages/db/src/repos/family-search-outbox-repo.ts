...
export class FamilySearchOutboxRepo extends Context.Tag('@repo/db/FamilySearchOutboxRepo')<
  FamilySearchOutboxRepo,
  {
    ...
    markFailed: (
      id: string,
      error: string
    ) => Effect.Effect<FamilySearchOutbox, SqlError | DBNotFoundError>;
    ...
    return {
      ...
      markFailed: (id, error) =>
        db
          ...
          .set({
            status: 'failed',
            attempts: sql`${familySearchOutbox.attempts} + 1`,
            lastError: error,
            updatedAt: new Date()
          })
          .where(eq(familySearchOutbox.id, id))
      ...
      markSupersededBefore: (cutoff) =>
        db
          ...
          .where(
            and(
              inArray(familySearchOutbox.status, unresolvedStatuses),
              lte(familySearchOutbox.createdAt, cutoff)
            )
          )
...
export const FamilySearchOutboxRepoDefault = FamilySearchOutboxRepoLive.pipe(
  Layer.provide(DrizzleLive)
);

export const makeFamilySearchOutboxRepoTest = (
  implementation: Context.Tag.Service<FamilySearchOutboxRepo>
