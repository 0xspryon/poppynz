...
import { makeSignupServiceTest, SignupAuthError, type SignupRole } from './signup.handler';
  ...
  const runtime = ManagedRuntime.make(
    Layer.mergeAll(
      ...
      makeKycDocumentRepoTest({
        ...
        submit: () =>
          Effect.fail(new DBNotFoundError({ entity: 'kycDocument', value: '' }) as never),
        updateExpiryDate: () =>
          Effect.fail(new DBNotFoundError({ entity: 'kycDocument', value: '' })),
      ...
      makeUserRepoTest({
        ...
        findByEmail: () => {
          if (options.existingUser) {
            return Effect.succeed(options.existingUser);
          }
...
describe('POST /auth/sign-up', () => {
  ...
  it('returns a safe invalid input response', async () => {
    ...
    expect(body.error.issues).toEqual([expect.objectContaining({ path: ['email'] })]);
  });
