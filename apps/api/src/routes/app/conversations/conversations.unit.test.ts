  ...
  return Layer.mergeAll(
    ...
    makeUserProfileRepoTest({
      ...
      findByUserId: (userId) =>
        Effect.succeed(
          userId === 'family-1'
            ? profile('family-1', 'Priya', 'Little Italy, Toronto')
            : profile('provider-1', 'Maria', 'Toronto')
        ),
...
describe('GET /conversations', () => {
  it('presents an ignored thread as pending to its initiator and counts unread', async () => {
    ...
    expect(
      result.conversations.map((entry) => [entry.id, entry.status, entry.awaitingMyResponse])
    ).toEqual([
      ['c-1', 'pending', false],
      ['c-2', 'active', false],
      ['c-3', 'pending', true]
  ...
  it('fails with ForbiddenError when the conversation read permission is missing', async () => {
    const exit = await Effect.runPromiseExit(
      listConversationsRouteProgram(new Headers()).pipe(
  ...
  it('reports the unread total on the badge endpoint', async () => {
    const layer = makeLayer({
      conversations: [
        { ...dummyConversationWithContext, status: 'pending', initiatorUserId: 'provider-1' }
      ]
    });
    const result = await Effect.runPromise(
      unreadCountRouteProgram(new Headers()).pipe(Effect.provide(layer))
    );
...
describe('GET /conversations/lookup', () => {
  it('returns null when the ignore cool-down has lapsed', async () => {
    ...
    const result = await Effect.runPromise(
      lookupConversationRouteProgram(
        makeContext({ query: { userId: 'provider-1' } }),
        new Headers()
      ).pipe(Effect.provide(layer))
    );
  ...
  it('routes the ignorer back to the thread as an answerable reach-out within the cool-down', async () => {
    ...
    const result = await Effect.runPromise(
      lookupConversationRouteProgram(
        makeContext({ query: { userId: 'family-1' } }),
        new Headers()
      ).pipe(Effect.provide(layer))
    );
  ...
  it('fails with ConversationNotFoundError for threads the viewer cannot see', async () => {
    const exit = await Effect.runPromiseExit(
      getConversationRouteProgram(
        makeContext({ params: { id: CONVERSATION_ID } }),
        new Headers()
      ).pipe(Effect.provide(makeLayer({ conversationWithContext: null })))
    );
    ...
    const result = await Effect.runPromise(
      sendMessageRouteProgram(
        makeContext({
          ...
          body: { body: '  School pickup at 3:30?  ' }
        }),
        new Headers()
      ).pipe(Effect.provide(layer))
    );
