...
export class ServiceCatalogueRepo extends Context.Tag('@repo/db/ServiceCatalogueRepo')<
  ServiceCatalogueRepo,
  {
    ...
    create: (
      input: ServiceCatalogueItemCreateInput
    ) => Effect.Effect<ServiceCatalogueItem, SqlError>;
    update: (
      id: string,
      input: ServiceCatalogueItemUpdateInput
    ) => Effect.Effect<ServiceCatalogueItem, SqlError | DBNotFoundError>;
...
export const ServiceCatalogueRepoLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return {
      listActive: () =>
        db.select().from(serviceCatalogueItem).where(isNull(serviceCatalogueItem.deletedAt)),
      listLive: () =>
        db
          .select()
          .from(serviceCatalogueItem)
          .where(
            and(isNull(serviceCatalogueItem.deletedAt), eq(serviceCatalogueItem.isLive, true))
          ),
      findActiveById: (id) =>
        db
          .select()
...
export const ServiceCatalogueRepoDefault = ServiceCatalogueRepoLive.pipe(
  Layer.provide(DrizzleLive)
);

export const makeServiceCatalogueRepoTest = (
  implementation: Context.Tag.Service<ServiceCatalogueRepo>
...
export const EmptyServiceCatalogueRepoTest = makeServiceCatalogueRepoTest({
  ...
  findActiveById: () =>
    Effect.fail(new DBNotFoundError({ entity: 'serviceCatalogueItem', value: '' })),
