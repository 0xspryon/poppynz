...
const providerUser = (overrides: Partial<User> = {}): User =>
  familyUser({
    id: "provider-1", name: "Maria S", email: "maria@example.com", role: "service-provider",
...
const profile = (userId: string, firstName: string, phoneNumber: string | null): SafeUserProfile => ({
  userId, email: `${firstName.toLowerCase()}@example.com`, role: userId === "family-1" ? "family" : "service-provider",
  language: "en", firstName, lastName: "Tester", gender: null, phoneNumber, dateOfBirth: null,
  address: null, city: "Toronto", postalCode: null, country: null, stateProvince: null, shortBio: null,
  googlePlaceId: null, latitude: null, longitude: null,
});

const offeredService = (overrides: Partial<ServiceOffered> = {}): ServiceOffered => ({
  id: SERVICE_ID, userId: "provider-1", catalogueServiceId: null,
...
const withContext = (
  ...
  ...contract,
  counterpartUserId: viewerUserId === contract.familyUserId ? contract.providerUserId : contract.familyUserId,
...
const makeLayer = (options: {
  ...
  return Layer.mergeAll(
    ...
    makeUserProfileRepoTest({
      ...
      findByUserId: (userId) =>
        Effect.succeed(
          userId === "family-1" ? profile("family-1", "Priya", null) : profile("provider-1", "Maria", "+1 416 555 0199"),
        ),
    ...
    makeContractRepoTest({
      ...
      withdrawAmendment: (versionId) => {
        ...
        return Effect.succeed(proposedVersion({ id: versionId, status: "withdrawn", decidedAt: new Date() }));
      },
...
describe("POST /contracts (create from conversation)", () => {
  it("creates a family-private draft from the family's active conversation, silently", async () => {
    ...
    const result = await Effect.runPromise(
      createContractRouteProgram(makeContext({ body: { conversationId: CONVERSATION_ID } }), new Headers()).pipe(
  ...
  it("refuses the provider side of the conversation — creation is structurally family-only", async () => {
    const exit = await Effect.runPromiseExit(
      createContractRouteProgram(makeContext({ body: { conversationId: CONVERSATION_ID } }), new Headers()).pipe(
        Effect.provide(makeLayer({ viewer: providerUser(), conversationById: activeConversation() })),
      ),
    );
  ...
  it("points at the existing contract when the conversation already has one", async () => {
    ...
    expect(getFailure(exit)).toMatchObject({ _tag: "ContractExistsError", contractId: CONTRACT_ID });
  });
...
describe("PUT /contracts/:id/terms", () => {
  const termsBody = {
    services: [{ serviceId: SERVICE_ID, rateCents: 2600, hoursPerWeek: 4, expectations: "Reading practice." }],
  ...
  it("snapshots name, listed rate and currency from the provider's listing", async () => {
    ...
    const result = await Effect.runPromise(
      saveTermsRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: termsBody }), new Headers()).pipe(
        Effect.provide(layer),
      ),
    );
    ...
    const exit = await Effect.runPromiseExit(
      saveTermsRouteProgram(
        makeContext({
          ...
          body: { ...termsBody, services: [{ ...termsBody.services[0], serviceId: OTHER_SERVICE_ID }] },
        }),
        new Headers(),
  ...
  it("refuses the provider side", async () => {
    const exit = await Effect.runPromiseExit(
      saveTermsRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: termsBody }), new Headers()).pipe(
        Effect.provide(makeLayer({ viewer: providerUser(), contractById: baseContract({ status: "declined" }) })),
      ),
    );
  ...
  it("refuses while a sent proposal is pending — withdraw first", async () => {
    const exit = await Effect.runPromiseExit(
      saveTermsRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: termsBody }), new Headers()).pipe(
        Effect.provide(
          makeLayer({ contractById: baseContract({ status: "proposed" }), versions: [proposedVersion()] }),
        ),
      ),
    );
  ...
  it("refuses on an active contract — new terms go through an amendment", async () => {
    const exit = await Effect.runPromiseExit(
      saveTermsRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: termsBody }), new Headers()).pipe(
        Effect.provide(makeLayer({ contractById: baseContract({ status: "active" }) })),
      ),
    );
    expect(getFailure(exit)).toMatchObject({ _tag: "ContractStateError" });
  });
  ...
  it("rejects duplicate service line items at validation", async () => {
    ...
    expect(getFailure(exit)).toMatchObject({ _tag: "RequestValidationError", code: "INVALID_CONTRACT_TERMS" });
  });

  it("rejects impossible calendar dates at validation", async () => {
    const exit = await Effect.runPromiseExit(
      saveTermsRouteProgram(
        makeContext({ params: { id: CONTRACT_ID }, body: { ...termsBody, startsOn: "2026-13-45" } }),
        new Headers(),
      ).pipe(Effect.provide(makeLayer({ contractById: baseContract(), versions: [draftVersion()] }))),
    );
    expect(getFailure(exit)).toMatchObject({ _tag: "RequestValidationError", code: "INVALID_CONTRACT_TERMS" });
  ...
  it("starts the next version when revising after a decline", async () => {
    ...
    expect(createdVersions[0]).toMatchObject({ version: 2, status: "draft", proposedByUserId: "family-1" });
  });
});
...
describe("POST /contracts/:id/send", () => {
  ...
  it("refuses an empty draft", async () => {
    const exit = await Effect.runPromiseExit(
      sendContractRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers()).pipe(
        Effect.provide(makeLayer({ contractById: baseContract(), versions: [draftVersion({ services: [] })] })),
      ),
    );
...
describe("POST /contracts/:id/withdraw", () => {
  it("returns a fresh proposal to draft, silently", async () => {
    ...
    const result = await Effect.runPromise(
      withdrawContractRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers()).pipe(
        Effect.provide(layer),
      ),
    );
...
describe("POST /contracts/:id/decline", () => {
  it("declines with a shared reason and notifies the family", async () => {
    ...
    const result = await Effect.runPromise(
      declineContractRouteProgram(
        makeContext({ params: { id: CONTRACT_ID }, body: { reason: "  Schedule no longer works.  " } }),
        new Headers(),
  ...
  it("still allows declining an expired proposal", async () => {
    ...
    const result = await Effect.runPromise(
      declineContractRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: {} }), new Headers()).pipe(
        Effect.provide(layer),
      ),
    );
    expect(result).toEqual({ id: CONTRACT_ID, status: "declined" });
  ...
  it("leaves an active contract untouched when an amendment is declined", async () => {
    ...
    expect(published[0].input).toMatchObject({ type: "contract.declined", payload: { isAmendment: true, reason: null } });
  });
...
describe("POST /contracts/:id/request-changes", () => {
  it("marks changes requested and hands back the conversation for the deep-link", async () => {
    ...
    expect(result).toEqual({ id: CONTRACT_ID, status: "changes_requested", conversationId: CONVERSATION_ID });
...
describe("POST /contracts/:id/amendments", () => {
  const amendmentBody = {
    services: [{ serviceId: SERVICE_ID, rateCents: 2800, hoursPerWeek: 6, expectations: "More hours." }],
...
describe("POST /contracts/:id/end", () => {
  it("gives 2 weeks' notice and notifies the counterpart with the last working day", async () => {
    ...
    const expectedEndsOn = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    ...
    expect(published).toEqual([
      {
        ...
        input: {
          type: "contract.ended",
          payload: { contractId: CONTRACT_ID, counterpartName: "Priya Tester", endsOn: expectedEndsOn },
        },
      },
    ]);
  });
  ...
  it("fails when the contract is not active", async () => {
    const exit = await Effect.runPromiseExit(
      endContractRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: {} }), new Headers()).pipe(
        Effect.provide(
...
describe("GET /contracts/:id", () => {
    ...
    const exit = await Effect.runPromiseExit(
      getContractRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers()).pipe(
        Effect.provide(
          makeLayer({
            viewer: providerUser(),
            contractWithContext: withContext(baseContract({ status: "draft" }), [draftVersion()], "provider-1"),
          }),
        ),
      ),
    );
  ...
  it("keeps contact details hidden pre-active and grants the receiver decision actions", async () => {
    const layer = makeLayer({
      viewer: providerUser(),
      contractWithContext: withContext(baseContract({ status: "proposed" }), [proposedVersion()], "provider-1"),
      conversationById: activeConversation(),
    });
  ...
  it("reveals contact details once active and offers amend/end to both sides", async () => {
    ...
    expect(contract.counterpartContact).toEqual({ email: "maria@example.com", phone: "+1 416 555 0199" });
  ...
  it("still presents 'ending' ON the last working day — payments run through it", async () => {
    ...
    const layer = makeLayer({
      contractWithContext: withContext(
        baseContract({ status: "ending", endsOn: today, endedByUserId: "family-1", endNoticedAt: new Date() }),
        [accepted],
        "family-1",
      ),
      conversationById: activeConversation(),
  ...
  it("presents an ending contract past its last working day as ended", async () => {
    ...
    const layer = makeLayer({
      contractWithContext: withContext(
        baseContract({ status: "ending", endsOn: "2026-08-01", endedByUserId: "provider-1", endNoticedAt: new Date() }),
        [accepted],
        "family-1",
      ),
      conversationById: activeConversation(),
...
describe("GET /contracts + badge", () => {
  it("translates a plain repo failure into ContractRepoError (500), not an escaped SqlError", async () => {
    const exit = await Effect.runPromiseExit(
      listContractsRouteProgram(new Headers()).pipe(
        Effect.provide(makeLayer({ listForUserFailsWith: new SqlError({ cause: new Error("boom") }) })),
      ),
    );
  ...
  it("never leaks a family's unsent revision draft into the provider's list row", async () => {
    ...
    const privateDraftV2 = draftVersion({
      id: "version-2",
      version: 2,
      services: [serviceItem({ serviceId: OTHER_SERVICE_ID, name: "Secret new plan", rateCents: 5000, hoursPerWeek: 30 })],
    });
    const layer = makeLayer({
      viewer: providerUser(),
      contracts: [
        withContext(baseContract({ status: "declined" }), [declinedV1, privateDraftV2], "provider-1"),
      ],
    });

    const result = await Effect.runPromise(listContractsRouteProgram(new Headers()).pipe(Effect.provide(layer)));
  ...
  it("lists the family's contracts with news flags and hides nothing from the owner", async () => {
    const declined = proposedVersion({ status: "declined", decidedAt: new Date(), declineReason: "Schedule" });
    const layer = makeLayer({
      contracts: [
        withContext(baseContract({ status: "draft" }), [draftVersion()], "family-1"),
        withContext(baseContract({ id: "0198a3b0-0000-7000-8000-00000000c002", status: "declined" }), [declined], "family-1"),
      ],
    });
    ...
    expect(result.contracts[0]).toMatchObject({ status: "draft", hasNews: false, awaitingYou: false });
  ...
  it("hides family-private drafts from the provider's list and counts what awaits them", async () => {
    ...
    const list = await Effect.runPromise(listContractsRouteProgram(new Headers()).pipe(Effect.provide(layer)));
    expect(list.contracts).toHaveLength(1);
    expect(list.contracts[0]).toMatchObject({ status: "proposed", awaitingYou: true, hasNews: true });

    const badge = await Effect.runPromise(contractsBadgeCountRouteProgram(new Headers()).pipe(Effect.provide(layer)));
...
describe("POST /contracts/:id/seen", () => {
  it("bumps the viewer's side marker", async () => {
    ...
    const result = await Effect.runPromise(
      markContractSeenRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers()).pipe(
        Effect.provide(layer),
      ),
    );
  ...
  it("hides the contract from non-participants", async () => {
    const exit = await Effect.runPromiseExit(
      markContractSeenRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers()).pipe(
        Effect.provide(
          makeLayer({ viewer: familyUser({ id: "family-2" }), contractById: baseContract({ status: "proposed" }) }),
        ),
      ),
    );
    expect(getFailure(exit)).toMatchObject({ _tag: "ContractNotFoundError" });
...
describe("authorization", () => {
  ...
  const cases: Array<[string, () => Effect.Effect<unknown, unknown, any>]> = [
    ["create", () => createContractRouteProgram(makeContext({ body: { conversationId: CONVERSATION_ID } }), new Headers())],
    ["list", () => listContractsRouteProgram(new Headers())],
    ["badge", () => contractsBadgeCountRouteProgram(new Headers())],
    ["detail", () => getContractRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers())],
    ["terms", () => saveTermsRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: { services: [] } }), new Headers())],
    ["send", () => sendContractRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers())],
    ["withdraw", () => withdrawContractRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers())],
    ["accept", () => acceptContractRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers())],
    ["decline", () => declineContractRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: {} }), new Headers())],
    ["request-changes", () => requestChangesRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers())],
    ["amend", () => amendContractRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: { services: [] } }), new Headers())],
    ["end", () => endContractRouteProgram(makeContext({ params: { id: CONTRACT_ID }, body: {} }), new Headers())],
    ["seen", () => markContractSeenRouteProgram(makeContext({ params: { id: CONTRACT_ID } }), new Headers())],
  ];

  it.each(cases)("fails %s with ForbiddenError when the contract permission is missing", async (_name, program) => {
