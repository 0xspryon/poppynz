import { DBNotFoundError, makeApprovalRepoTest, makeFamilySearchOutboxRepoTest, makeFamilySearchRepoTest, makeSessionRepoTest, makeUserProfileRepoTest, makeUserRepoTest, type Approval, type FamilySearchCandidate, type Session, type User } from "@repo/db";
...
const makeLayer = (options: {
  ...
  return Layer.mergeAll(
    ...
    makeFamilySearchOutboxRepoTest({
      ...
      markProcessing: (id) => Effect.fail(new DBNotFoundError({ entity: "familySearchOutbox", value: id })),
      markProcessed: (id) => Effect.fail(new DBNotFoundError({ entity: "familySearchOutbox", value: id })),
      markFailed: (id) => Effect.fail(new DBNotFoundError({ entity: "familySearchOutbox", value: id })),
    ...
    makeFamilySearchIndexTest({
      ...
      searchFamilies: (input) => Effect.succeed({
...
describe("family search route program", () => {
  ...
  it("returns public family search results", async () => {
    const result = await Effect.runPromise(searchFamiliesRouteProgram(new Headers(), { q: "childcare" }).pipe(Effect.provide(makeLayer())));
  ...
  it("never exposes rate fields on family results", async () => {
    const result = await Effect.runPromise(searchFamiliesRouteProgram(new Headers(), {}).pipe(Effect.provide(makeLayer())));
    ...
    const result = await Effect.runPromise(
      searchFamiliesRouteProgram(new Headers(), {}).pipe(
        Effect.provide(makeLayer({
          documents: [familyDocument({ displayName: "Stale Name", firstName: "Stale", lastName: "Name", shortBio: "Stale bio" })],
  ...
  it("rejects lat/lng query parameters", async () => {
    const exit = await Effect.runPromise(searchFamiliesRouteProgram(new Headers(), { lat: "43", lng: "-79" }).pipe(Effect.provide(makeLayer()), Effect.exit));
  ...
  it("requires saved user location for radius search", async () => {
    const exit = await Effect.runPromise(searchFamiliesRouteProgram(new Headers(), { radiusKm: "10" }).pipe(Effect.provide(makeLayer({ hasLocation: false })), Effect.exit));

    expect(getFailure(exit)._tag).toBe("FamilySearchRequestValidationError");
  });

  it("requires family search read permission", async () => {
    const exit = await Effect.runPromise(searchFamiliesRouteProgram(new Headers(), {}).pipe(Effect.provide(makeLayer({ hasPermission: false })), Effect.exit));

    expect(getFailure(exit)._tag).toBe("ForbiddenError");
  });

  it("rejects providers without a current approval", async () => {
    const exit = await Effect.runPromise(searchFamiliesRouteProgram(new Headers(), {}).pipe(Effect.provide(makeLayer({ approved: false })), Effect.exit));
  ...
  it("lets admins search without an approval row", async () => {
    const result = await Effect.runPromise(
      searchFamiliesRouteProgram(new Headers(), {}).pipe(
        Effect.provide(makeLayer({ user: user({ id: "provider-1", role: "admin" }), approved: false })),
      ),
    );
...
describe("family detail route program", () => {
  ...
  it("returns the family detail with needed services and no rates", async () => {
    const result = await Effect.runPromise(getFamilyRouteProgram(new Headers(), "family-1").pipe(Effect.provide(makeLayer())));
    ...
    expect(result.services).toEqual([
      { id: "00000000-0000-7000-8000-000000000003", name: "Childcare", description: "After-school care" },
    ]);
  });
  ...
  it("hides families that fail the eligibility gate", async () => {
    const exit = await Effect.runPromise(
      getFamilyRouteProgram(new Headers(), "family-1").pipe(
        Effect.provide(makeLayer({ candidates: [candidate("family-1", { banned: true, banExpires: null })] })),
        Effect.exit,
      ),
    );
  ...
  it("rejects unapproved providers", async () => {
    const exit = await Effect.runPromise(getFamilyRouteProgram(new Headers(), "family-1").pipe(Effect.provide(makeLayer({ approved: false })), Effect.exit));
