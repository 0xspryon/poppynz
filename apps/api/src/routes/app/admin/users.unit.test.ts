...
const admin = (): User => ({
  id: "admin-1", name: "Alex Chen", email: "alex@poppynz.com", emailVerified: true, image: null,
  createdAt: new Date("2026-01-02T00:00:00.000Z"), updatedAt: new Date("2026-01-02T00:00:00.000Z"),
...
const directoryUser = (overrides: Partial<DirectoryUser> = {}): DirectoryUser => ({
  id: "user-1", email: "maria@email.com", name: "maria@email.com", role: "service-provider",
  banned: false, banReason: null, createdAt: new Date("2026-06-12T00:00:00.000Z"),
...
describe("GET /admin/users", () => {
  it("returns the directory with profile names preferred over the auth name", async () => {
      ...
      listAdminUsersRouteProgram(new Headers()).pipe(
        Effect.provide(
          makeLayer({
            users: [
              directoryUser(),
              directoryUser({ id: "user-2", email: "jordan@email.com", name: "Jordan", firstName: null, lastName: null, role: "family" }),
              directoryUser({ id: "user-3", email: "banned@email.com", banned: true, banReason: "spam" }),
            ],
          }),
        ),
      ),
    ...
    expect(result.users[0]).toMatchObject({ name: "Maria Santos", role: "service-provider", banned: false });
