...
class TcDraftNotFoundError extends Data.TaggedError("TcDraftNotFoundError")<{ documentId: string }> { }
...
function mapTcRepoError<A, R>(effect: Effect.Effect<A, SqlError, R>): Effect.Effect<A, TcRepoError, R>;
...
export const listPublishedTcsRouteProgram = (roleParam: string) =>
  Effect.gen(function*() {
    ...
    const published = yield* repo.listPublishedForRole(role)
...
export const listPendingTcsRouteProgram = (headers: Headers) =>
  Effect.gen(function*() {
    ...
    const repo = yield* TcDocumentRepo;
    const pending = yield* repo.listPendingForUser(userAndSession.user.id, role)
      .pipe((errors) => mapTcRepoError(errors));
...
export const acceptTcsRouteProgram = (c: HonoContext<HonoEnv>, headers: Headers) =>
  Effect.gen(function*() {
    ...
    const items = yield* Effect.forEach(input.acceptances, (acceptance) =>
      Effect.gen(function*() {
        const document = yield* repo.findActiveBySlug(acceptance.slug)
          .pipe((errors) => mapTcRepoError(errors));
        const latest = yield* repo.findLatestPublished(document.id)
          .pipe((errors) => mapTcRepoError(errors));
    ...
    const accepted = yield* repo.insertAcceptances(userAndSession.user.id, items)
      .pipe((errors) => mapTcRepoError(errors));
...
export const listAdminTcsRouteProgram = (headers: Headers) =>
  Effect.gen(function*() {
    ...
    const repo = yield* TcDocumentRepo;
    const documents = yield* repo.listActive()
      .pipe((errors) => mapTcRepoError(errors));
    const versions = yield* repo.listVersionsByDocumentIds(documents.map((document) => document.id))
      .pipe((errors) => mapTcRepoError(errors));
...
export const getAdminTcRouteProgram = (headers: Headers, id: string) =>
  Effect.gen(function*() {
    ...
    yield* requirePermissions(headers, { tcs: ["write"] })(authenticated);
    const repo = yield* TcDocumentRepo;
    const document = yield* repo.findActiveById(id)
      .pipe((errors) => mapTcRepoError(errors));
    const versions = yield* repo.listVersions(document.id)
      .pipe((errors) => mapTcRepoError(errors));
...
export const createTcDocumentRouteProgram = (c: HonoContext<HonoEnv>, headers: Headers) =>
  Effect.gen(function*() {
    ...
    const document = yield* repo.createDocument(input)
      .pipe((errors) => mapTcRepoError(errors));
    return toTcDocumentResponse(document);
  });

export const updateTcDocumentRouteProgram = (c: HonoContext<HonoEnv>, headers: Headers, id: string) =>
  Effect.gen(function*() {
    const rawBody = yield* parseJsonBody(c, tcJsonError);
    ...
    yield* requirePermissions(headers, { tcs: ["write"] })(authenticated);
    const repo = yield* TcDocumentRepo;
    const document = yield* repo.updateDocument(id, input)
      .pipe((errors) => mapTcRepoError(errors));
    return toTcDocumentResponse(document);
  });
...
export const deleteTcDocumentRouteProgram = (headers: Headers, id: string) =>
  Effect.gen(function*() {
    ...
    yield* requirePermissions(headers, { tcs: ["write"] })(authenticated);
    const repo = yield* TcDocumentRepo;
    const document = yield* repo.softDeleteDocument(id)
      .pipe((errors) => mapTcRepoError(errors));
    return toTcDocumentResponse(document);
  });
...
export const updateTcDraftRouteProgram = (c: HonoContext<HonoEnv>, headers: Headers, id: string) =>
  Effect.gen(function*() {
    ...
    yield* requirePermissions(headers, { tcs: ["write"] })(authenticated);
    const repo = yield* TcDocumentRepo;
    const draft = yield* repo.updateDraft(id, input)
      .pipe((errors) => mapTcRepoError(errors));
    return toTcVersionResponse(draft);
  });
...
export const publishTcDraftRouteProgram = (headers: Headers, id: string) =>
  Effect.gen(function*() {
    ...
    yield* requirePermissions(headers, { tcs: ["write"] })(authenticated);
    const repo = yield* TcDocumentRepo;
    const published = yield* repo.publishDraft(id)
      .pipe((errors) => mapTcRepoError(errors));
...
const tcErrorToResponse = (c: HonoContext<HonoEnv>, error: TcRouteError) => {
  ...
  switch (error._tag) {
    case "TcDocumentNotFoundError":
      return c.json(
        { error: { code: "TC_DOCUMENT_NOT_FOUND" as const, message: "Terms and conditions document was not found." } },
        404,
      );
    ...
    case "TcDraftExistsError":
      return c.json(
        { error: { code: "TC_DRAFT_EXISTS" as const, message: "The document already has an open draft." } },
        409,
      );
    case "TcSlugTakenError":
      return c.json(
        { error: { code: "TC_SLUG_TAKEN" as const, message: "A document with this slug already exists." } },
        409,
      );
    case "TcVersionStaleError":
      return c.json(
        { error: { code: "TC_VERSION_STALE" as const, message: "A newer version was published. Reload the terms and accept again." } },
        409,
      );
    case "TcRepoError":
      return c.json(
        { error: { code: "TC_REPO_ERROR" as const, message: "Unable to process terms and conditions request." } },
        500,
      );
    default:
