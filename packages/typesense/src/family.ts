...
export class FamilySearchIndex extends Context.Tag('@repo/typesense/FamilySearchIndex')<
  FamilySearchIndex,
  {
    ...
    searchFamilies: (
      input: FamilySearchInput
    ) => Effect.Effect<FamilySearchResult, FamilySearchIndexError>;
    listCityFacets: (
      input: FamilySearchInput
    ) => Effect.Effect<Array<FamilyCityFacet>, FamilySearchIndexError>;
    getFamily: (
      userId: string
    ) => Effect.Effect<FamilySearchDocument, FamilySearchIndexError | DBNotFoundError>;
    reindexAllFamilies: () => Effect.Effect<
      { indexed: number; deletedStale: number },
      FamilySearchIndexError
    >;
  }
>() {}
...
const makeFamilySearchIndex = (config: {
  host: string;
  port: number;
  protocol: string;
  apiKey: string;
  familyCollection: string;
  familyCollectionAlias: string;
  familyCollectionVersion: string;
}) => {
    ...
    const reindexAllFamilies = () =>
      Effect.gen(function* () {
          ...
          (userId) =>
            Effect.gen(function* () {
              const candidate = yield* repo
                ...
                .pipe(
                  Effect.mapError(
                    (cause) => new FamilySearchIndexError({ operation: 'reindex', cause })
                  )
                );
        ...
        Effect.catchAllCause((cause) =>
          Effect.fail(
            new FamilySearchIndexError({ operation: 'reindex', cause: Cause.pretty(cause) })
          )
        )
      );
...
export const FamilySearchIndexDefault = FamilySearchIndexLive.pipe(
  Layer.provide(FamilySearchRepoDefault)
);
...
export const getFamilySearchMinRadiusKm = familySearchConfig.pipe(
