...
export class ProviderSearchOutboxRepo extends Context.Tag('@repo/db/ProviderSearchOutboxRepo')<
  ProviderSearchOutboxRepo,
  {
    ...
    markFailed: (
      id: string,
      error: string
    ) => Effect.Effect<ProviderSearchOutbox, SqlError | DBNotFoundError>;
    ...
    return {
      ...
      markFailed: (id, error) =>
        db
          ...
          .set({
            status: 'failed',
            attempts: sql`${providerSearchOutbox.attempts} + 1`,
            lastError: error,
            updatedAt: new Date()
          })
          .where(eq(providerSearchOutbox.id, id))
      ...
      markSupersededBefore: (cutoff) =>
        db
          ...
          .where(
            and(
              inArray(providerSearchOutbox.status, unresolvedStatuses),
              lte(providerSearchOutbox.createdAt, cutoff)
            )
          )
...
export const ProviderSearchOutboxRepoDefault = ProviderSearchOutboxRepoLive.pipe(
  Layer.provide(DrizzleLive)
);

export const makeProviderSearchOutboxRepoTest = (
  implementation: Context.Tag.Service<ProviderSearchOutboxRepo>
