...
import { signinJsonError, validateSigninInput, type SigninInput } from './signin.validator';
import {
...
export class SigninService extends Context.Tag('@api/routes/auth/signin/SigninService')<
  SigninService,
  {
    sendSigninLink: (input: {
      email: string;
      headers: Headers;
    }) => Effect.Effect<void, SigninAuthError>;
  }
>() {}
...
const isSigninError = (error: unknown): error is SigninError =>
  error instanceof SigninUserLookupError ||
  error instanceof SigninUserNotFoundError ||
  error instanceof SigninAuthError;
...
const exitToResponse = <TResult>(
  c: HonoContext<HonoEnv>,
  exit: Exit.Exit<TResult, SigninRouteError>
) =>
  Exit.match(exit, {
