...
const makeLayer = (
  ...
  const documentTypes = options.documentTypes ?? [
    documentType(),
    documentType({ id: 'optional-doc', name: 'Driving License', isOptional: true })
  ];
    ...
    makeApprovalRepoTest({
      ...
      findCurrentByUserId: (userId) =>
        options.approval === null
          ...
          : Effect.succeed(options.approval ?? approval({ userId })),
...
describe('profile programs', () => {
  ...
  it('replaces the stored image key with a presigned URL', async () => {
    const result = await Effect.runPromise(
      getProfileProgram(userAndSession).pipe(
        Effect.provide(
          makeLayer({ profile: profile({ image: 'users/user-1/public/profile-pictures/pic.png' }) })
        )
      )
    );
  ...
  it('returns a null image when no profile picture is stored', async () => {
    const result = await Effect.runPromise(
      getProfileProgram(userAndSession).pipe(Effect.provide(makeLayer()))
    );
  ...
  it('omits service-provider onboarding state for family users', async () => {
    const result = await Effect.runPromise(
      getProfileProgram(userAndSession).pipe(
        Effect.provide(makeLayer({ profile: profile({ role: 'family' }) }))
      )
    );
  ...
  it('translates missing profile to ProfileNotFoundError', async () => {
    const exit = await Effect.runPromise(
      getProfileProgram(userAndSession).pipe(
        Effect.provide(makeLayer({ profile: null })),
        Effect.exit
      )
    );
  ...
  it('translates profile SQL failures to ProfileRepoError', async () => {
    const exit = await Effect.runPromise(
      getProfileProgram(userAndSession).pipe(
        Effect.provide(makeLayer({ profileError: new SqlError({ message: 'db down' }) })),
        Effect.exit
      )
    );
