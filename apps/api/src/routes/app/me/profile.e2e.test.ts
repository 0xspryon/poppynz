...
const makeApp = (
  ...
  const user =
    options.user === undefined
      ? makeUser({ id: profile.userId, email: profile.email, role: profile.role })
      : options.user;
  const session =
    options.session === undefined ? makeSession({ userId: profile.userId }) : options.session;
  const approval =
    options.approval === undefined ? makeApproval({ userId: profile.userId }) : options.approval;
  const approvalRequest =
    options.approvalRequest === undefined
      ? makeApprovalRequest({ userId: profile.userId })
      : options.approvalRequest;
  const documentTypes = options.documentTypes ?? [
    makeDocumentType(),
    makeDocumentType({ id: 'optional-doc', name: 'Driving License', isOptional: true })
  ];
  const kycDocuments = options.kycDocuments ?? [makeKycDocument({ userId: profile.userId })];
  const servicesOffered = options.servicesOffered ?? [
    makeServiceOffered({ userId: profile.userId })
  ];
    ...
    Layer.mergeAll(
        ...
        getSession: () =>
          Effect.succeed(
            options.authSession === undefined
              ? { user: { id: profile.userId }, session: { id: 'session-1' } }
              : options.authSession
          ),
      ...
      makeUserRepoTest({
        ...
        findByEmail: () =>
          user
            ? Effect.succeed(user)
            : Effect.fail(new DBNotFoundError({ entity: 'user', value: '' }))
      }),
      makeSessionRepoTest({
        findById: (id) =>
          session?.id === id
      ...
      makeApprovalRequestRepoTest({
        ...
        listByUserId: (userId) =>
          Effect.succeed(approvalRequest?.userId === userId ? [approvalRequest] : []),
        findById: (id) =>
          approvalRequest?.id === id
            ? Effect.succeed(approvalRequest)
            : Effect.fail(new DBNotFoundError({ entity: 'approvalRequest', value: id })),
        findSubmittedByUserId: (userId) =>
          approvalRequest?.userId === userId && approvalRequest.status === 'submitted'
            ? Effect.succeed(approvalRequest)
        ...
        markApproved: (id) =>
          approvalRequest?.id === id
            ? Effect.succeed({ ...approvalRequest, status: 'approved' })
            : Effect.fail(new DBNotFoundError({ entity: 'approvalRequest', value: id })),
        reject: (id) =>
          approvalRequest?.id === id
      ...
      makeKycDocumentRepoTest({
        findByIdWithType: () =>
          Effect.fail(new DBNotFoundError({ entity: 'kycDocument', value: '' })),
        findByUserId: (userId) =>
          Effect.succeed(
            kycDocuments.filter(
              (document) => document.userId === userId && document.deletedAt === null
            )
          ),
        findByUserIdWithTypes: (userId) =>
          Effect.succeed(
            kycDocuments
              ...
              .map((document) => ({
                ...document,
                documentType: documentTypes.find((type) => type.id === document.documentTypeId)!
              }))
          ),
        ...
        listByUserId: (userId) =>
          Effect.succeed(
            servicesOffered.filter(
              (service) => service.userId === userId && service.deletedAt === null
            )
          ),
...
describe('/me/profile', () => {
  ...
  it('returns missing document and service warnings', async () => {
    ...
    expect(res.status).toBe(200);
    expect(body.missingRequiredDocuments).toEqual([
      { documentTypeId: 'document-type-1', name: 'Government ID' }
    ]);
  ...
  it('omits service-provider onboarding state for family users', async () => {
    const app = makeApp({
      profile: makeProfile({ role: 'family' }),
      user: makeUser({ role: 'family' })
    });
    const res = await app.request('/api/v1/me/profile');
    const body = await res.json();
  ...
  it('updates location from a Google place id', async () => {
    ...
    expect(updates).toEqual([
      expect.objectContaining({ googlePlaceId: 'place-1', latitude: 43.6532, longitude: -79.3832 })
    ]);
  });
});
