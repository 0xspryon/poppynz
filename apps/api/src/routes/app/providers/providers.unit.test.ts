...
const providerDocument = (
  overrides: Partial<ProviderSearchDocument> = {}
  ...
  return Layer.mergeAll(
    ...
    makeProviderSearchRepoTest({
      ...
      listServiceProviderUserIds: () =>
        Effect.succeed(candidates.map((entry) => entry.profile.userId))
    }),
    makeProviderSearchOutboxRepoTest({
...
describe('provider search route program', () => {
  ...
  it('returns public provider search results', async () => {
    const result = await Effect.runPromise(
      searchProvidersRouteProgram(new Headers(), { q: 'childcare' }).pipe(
  ...
  it('replaces the stored image key with a presigned URL', async () => {
    const result = await Effect.runPromise(
      searchProvidersRouteProgram(new Headers(), {}).pipe(Effect.provide(makeLayer()))
    );
  ...
  it('rejects lat/lng query parameters', async () => {
    const exit = await Effect.runPromise(
      searchProvidersRouteProgram(new Headers(), { lat: '43', lng: '-79' }).pipe(
        Effect.provide(makeLayer()),
        Effect.exit
      )
    );
  ...
  it('requires saved user location for radius search', async () => {
    const exit = await Effect.runPromise(
      searchProvidersRouteProgram(new Headers(), { radiusKm: '10' }).pipe(
