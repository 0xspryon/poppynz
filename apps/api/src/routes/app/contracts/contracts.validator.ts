...
export const validateContractCreateInput = validateInput(contractCreateSchema, contractCreateValidationError);
...
const isoCalendarDate = Schema.String.pipe(
  ...
  Schema.filter((value) => !Number.isNaN(new Date(`${value}T00:00:00Z`).getTime()) &&
...
export const contractTermsSchema = Schema.Struct({
  services: Schema.Array(
    ...
    Schema.maxItems(20),
    Schema.filter((services) => new Set(services.map((service) => service.serviceId)).size === services.length, {
...
export const validateContractTermsInput = validateInput(contractTermsSchema, contractTermsValidationError);
...
export const validateContractDeclineInput = validateInput(contractDeclineSchema, contractDeclineValidationError);
...
export const validateContractEndInput = validateInput(contractEndSchema, contractEndValidationError);
