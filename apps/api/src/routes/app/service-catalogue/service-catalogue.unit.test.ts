...
describe('service catalogue route programs', () => {
  it('lists only live items for the provider-facing endpoint', async () => {
    const items = [
      item(),
      item({ id: 'item-2', name: 'Homework tutoring', category: 'Tutoring', isLive: false })
    ];
    const result = await Effect.runPromise(
      listLiveServiceCatalogueRouteProgram(new Headers()).pipe(Effect.provide(makeLayer({ items })))
    );
  ...
  it('lists hidden items too on the admin endpoint', async () => {
    ...
    const result = await Effect.runPromise(
      listAdminServiceCatalogueRouteProgram(new Headers()).pipe(
        Effect.provide(makeLayer({ items }))
      )
    );
  ...
  it('updates an item', async () => {
    ...
    const result = await Effect.runPromise(
      updateServiceCatalogueItemRouteProgram(
        contextWithJson({ isLive: false }),
        new Headers(),
        'item-1'
      ).pipe(Effect.provide(makeLayer({ onUpdate: (_id, input) => updates.push(input) })))
    );
  ...
  it('soft deletes an item', async () => {
    const result = await Effect.runPromise(
      deleteServiceCatalogueItemRouteProgram(new Headers(), 'item-1').pipe(
  ...
  it('maps SqlError to ServiceCatalogueRepoError on create', async () => {
    const exit = await Effect.runPromise(
      createServiceCatalogueItemRouteProgram(
        ...
        Effect.provide(
          makeLayer({ createError: new SqlError({ cause: new Error('boom'), message: 'boom' }) })
        ),
        Effect.exit
      )
    );
