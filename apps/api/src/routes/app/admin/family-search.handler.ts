...
type AdminFamilySearchRouteError = Effect.Effect.Error<ReturnType<typeof scheduleFamilySearchReindexRouteProgram>>;
...
const errorToResponse = (c: HonoContext<HonoEnv>, error: AdminFamilySearchRouteError) => {
  switch (error._tag) {
    ...
    case "FamilySearchQueueError":
      return c.json({ error: { code: "FAMILY_SEARCH_REINDEX_SCHEDULE_FAILED" as const, message: "Unable to schedule family search reindex." } }, 502);
    default:
...
export async function scheduleFamilySearchReindexHandler(c: HonoContext<HonoEnv>) {
  ...
  const exit = await runtime.runPromiseExit(scheduleFamilySearchReindexRouteProgram(c.req.raw.headers));
