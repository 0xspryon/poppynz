...
export const createApprovalProgram = (userAndSession: UserAndSession, input: ApprovalInput) =>
  Effect.gen(function*() {
    ...
    const approvalRequest = yield* approvalRequestRepo
      .findById(input.approvalRequestId)
      .pipe(
        Effect.catchTags({
          SqlError: (cause) => Effect.fail(new ApprovalRepoError({ cause })),
          DBNotFoundError: () => Effect.fail(new ApprovalRequestNotFoundError({ id: input.approvalRequestId })),
        }),
      );
...
export async function createApprovalHandler(c: HonoContext<HonoEnv>) {
  ...
  const exit = await runtime.runPromiseExit(
    Effect.gen(function*() {
      ...
      const authenticated = yield* authenticate(headers);
      const userAndSession = yield* requirePermissions(headers, { approval: ["write"] })(authenticated);
