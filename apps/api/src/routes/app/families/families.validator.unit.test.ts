...
import { validateFamilySearchQuery } from "./families.validator";

const run = (query: Record<string, string | undefined>) =>
  Effect.runPromiseExit(validateFamilySearchQuery(query));
...
describe("family search pagination validation", () => {
  it("defaults page and perPage when absent", async () => {
    const result = await Effect.runPromise(validateFamilySearchQuery({}));
    expect(result.page).toBe(1);
    expect(result.perPage).toBe(20);
  });

  it("accepts values at the maximum bounds", async () => {
    const result = await Effect.runPromise(validateFamilySearchQuery({ page: "100", perPage: "50" }));
  ...
  it("rejects trailing-garbage numerics that parseInt would accept", async () => {
    expect(failureMessage(await run({ perPage: "20x" }))).toBe("perPage must be a positive integer.");
  });
  ...
  it("rejects price sorts that only exist on provider search", async () => {
    expect(failureMessage(await run({ sort: "price_asc" }))).toBe("sort is not supported.");
  });

  it("requires radiusKm for distance sort", async () => {
    expect(failureMessage(await run({ sort: "distance" }))).toBe("sort=distance requires radiusKm.");
  });
});
