...
const makeLayer = (
    ...
    activeDocuments()
      ...
      .flatMap((document) => {
        const latest = versions
          .filter(
            (candidate) => candidate.documentId === document.id && candidate.publishedAt !== null
          )
  ...
  return Layer.mergeAll(
    ...
    makeTcDocumentRepoTest({
      listActive: () =>
        options.listError ? Effect.fail(options.listError) : Effect.succeed(activeDocuments()),
      findActiveById: (id) => {
      ...
      updateDraft: (documentId, input) => {
        const draft = versions.find(
          (candidate) => candidate.documentId === documentId && candidate.publishedAt === null
        );
        if (!draft)
          return Effect.fail(
            new DBNotFoundError({ entity: 'tcDocumentVersion', value: documentId })
          );
...
describe('tcs route programs', () => {
  it('lists published documents for a role, including all-audience documents', async () => {
    ...
    const result = await Effect.runPromise(
      listPublishedTcsRouteProgram('service-provider').pipe(
        Effect.provide(makeLayer({ documents, versions }))
      )
    );
  ...
  it('lists documents with their version history for admins', async () => {
    const versions = [
      version(),
      version({ id: 'version-2', version: 2, publishedAt: null, description: 'WIP' })
    ];
    const result = await Effect.runPromise(
  ...
  it('updates the open draft only', async () => {
    ...
    const result = await Effect.runPromise(
      updateTcDraftRouteProgram(
        contextWithJson({ content: '# Edited' }),
        new Headers(),
        'doc-1'
      ).pipe(Effect.provide(makeLayer({ versions })))
    );
  ...
  it('maps a missing draft to TcDraftNotFoundError on publish', async () => {
    const exit = await Effect.runPromise(
      publishTcDraftRouteProgram(new Headers(), 'doc-1').pipe(
        Effect.provide(makeLayer()),
        Effect.exit
      )
    );
  ...
  it('maps SqlError to TcRepoError on the admin list', async () => {
    const exit = await Effect.runPromise(
      listAdminTcsRouteProgram(new Headers()).pipe(
        Effect.provide(
          makeLayer({ listError: new SqlError({ cause: new Error('boom'), message: 'boom' }) })
        ),
        Effect.exit
      )
    );
