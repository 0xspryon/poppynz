...
class ServiceCatalogueNotFoundError extends Data.TaggedError('ServiceCatalogueNotFoundError')<{
  id: string;
}> {}
class ServiceCatalogueRepoError extends Data.TaggedError('ServiceCatalogueRepoError')<{
  cause: SqlError;
}> {}

function mapCatalogueRepoError<A, R>(
  effect: Effect.Effect<A, SqlError, R>
): Effect.Effect<A, ServiceCatalogueRepoError, R>;
function mapCatalogueRepoError<A, R>(
  ...
  return effect.pipe(
    Effect.catchTags({
      ...
      DBNotFoundError: (cause) =>
        Effect.fail(new ServiceCatalogueNotFoundError({ id: cause.value }))
    })
  );
}
...
export const listAdminServiceCatalogueRouteProgram = (headers: Headers) =>
  Effect.gen(function* () {
    ...
    const repo = yield* ServiceCatalogueRepo;
    const items = yield* repo.listActive().pipe((error) => mapCatalogueRepoError(error));
    return items.map(toCatalogueItemResponse);
  });
...
export const updateServiceCatalogueItemRouteProgram = (
...
const serviceCatalogueErrorToResponse = (
  c: HonoContext<HonoEnv>,
  error: ServiceCatalogueRouteError
) => {
  ...
  switch (error._tag) {
    ...
    case 'ServiceCatalogueRepoError':
      return c.json(
        {
          error: {
            code: 'SERVICE_CATALOGUE_REPO_ERROR' as const,
            message: 'Unable to process service catalogue request.'
          }
        },
        500
      );
...
const exitToResponse = <T>(
  c: HonoContext<HonoEnv>,
  exit: Exit.Exit<T, ServiceCatalogueRouteError>
) =>
  Exit.match(exit, {
