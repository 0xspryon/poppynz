...
import {
  ...
  listReferralsRouteProgram,
} from "./referrals.handler";

const user = (overrides: Partial<User> = {}): User => ({
  id: "referrer-1", name: "Referrer", email: "referrer@example.com", emailVerified: true, image: null,
...
const session = (): Session => ({
  ...
  createdAt: new Date("2026-06-12T00:00:00.000Z"), updatedAt: new Date("2026-06-12T00:00:00.000Z"),
  ipAddress: null, userAgent: null, userId: "referrer-1", impersonatedBy: null, activeOrganizationId: null,
});

const profile = (): SafeUserProfile => ({
  userId: "referrer-1", email: "referrer@example.com", role: "service-provider", language: "en",
  firstName: "Maria", lastName: "Santos", gender: null, phoneNumber: null, dateOfBirth: null,
  address: null, city: null, postalCode: null, country: null, stateProvince: null, shortBio: null,
...
const makeLayer = (options: {
  ...
  onCreateReferral?: (input: { referrerUserId: string; email: string; role: string; expiresAt: Date }) => void;
  onCreateIntent?: (input: { email: string; role: string; language: string; expiresAt: Date }) => void;
    ...
    makeSignupIntentRepoTest({
      create: (input) => {
        ...
        return Effect.succeed({
          id: "intent-1", email: input.email, role: input.role, language: input.language,
          expiresAt: input.expiresAt, consumedAt: null, createdAt: new Date(),
...
describe("GET /referrals", () => {
  it("derives statuses and stats from the referral rows", async () => {
    ...
    const layer = makeLayer({
      referrals: [
        referredEntry({
          id: "r-joined", referredUserId: "referred-1", joinedAt: new Date("2026-07-02T00:00:00.000Z"),
          referredFirstName: "Rosa", referredLastName: "Benitez", referredVerified: true, expiresAt: past,
        }),
...
describe("POST /referrals", () => {
  it("creates a signup intent and referral, then sends the invite", async () => {
    ...
    expect(created).toEqual([
      expect.objectContaining({ email: "rosa.friend@gmail.com", role: "service-provider", referrerUserId: "referrer-1" }),
    ]);
    expect(intents).toEqual([
      expect.objectContaining({ email: "rosa.friend@gmail.com", role: "service-provider", language: "en" }),
    ]);
    expect(mails).toHaveLength(1);
    expect(mails[0]?.inviterName).toBe("Maria Santos");
    expect(mails[0]?.link).toContain("/auth/sign-up?email=rosa.friend%40gmail.com&role=service-provider");
  ...
  it("rejects invalid input before touching auth", async () => {
    ...
    expect(getFailure(exit)).toMatchObject({ _tag: "RequestValidationError", code: "INVALID_REFERRAL_INPUT" });
  });
});
